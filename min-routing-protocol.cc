  ...
  while (N2.begin () != N2.end ())
    {
      ...
      //std::map<std::vector<const NeighborTuple *> ,int> reachability1;
      std::set<int> rs;
      for (NeighborSet::iterator it = N.begin (); it != N.end (); it++)
       ...
       //add//
      std::map<double, std::vector<const NeighborTuple *> > reachwillness;
      for (NeighborSet::iterator it = N.begin (); it != N.end (); it++)
        {
          NeighborTuple const &nb_tuple = *it;
          double rw = 0;
          for (TwoHopNeighborSet::iterator it2 = N2.begin (); it2 != N2.end (); it2++)
            {
              ...
              if (nb_tuple.neighborMainAddr == nb2hop_tuple.neighborMainAddr)
                {
                  rw+=nb_tuple.willingness*nb2hop_tuple.willingness;
                }
            }
          reachwillness.push_back (1/rw,&nb_tuple);
        }
...
//add
      ...
      for (std::map<double, std::vector<const NeighborTuple *> > ::iterator it2 = reachwillness.begin ();
               it2 != reachwillness.end (); it2++)
            {
              const NeighborTuple *nb_tuple = *it2.second;
              for (std::map<int, std::vector<const NeighborTuple *> > ::iterator it1=reachability.begin ();
              	it1 != reachability.end (); it1++)
              	{
              		int reah_n=0；
              		if (*nb_tuple==*t1.second)
    				{
        			   reah_n = (*t1.second).first;
        		    }
              		if (max == NULL || reah_n > max_reah)
                	{
                  		max = nb_tuple;
                  		max_reah=reah_n;

                  	}
             		else if (reah_n ==max_reah)
             		{
                     if (Degree (*nb_tuple) > Degree (*max))
                        {
                          max = nb_tuple;
                                      ...
                                      const min_routing::MessageHeader::Hello &hello)
{
  ...
  NeighborTuple *nb_tuple_next = m_state.FindNeighborTuple (msg.GetOriginatorAddress ()); //add
  ...
  if (nb_tuple != NULL)
    {
	   ...
	   int N_willingness=3;
       double d=sqrt(pow((x_mpos-x_npos),mi)+pow((y_mpos-y_npos),mi));       //distance

       double d_next=sqrt(pow((x_mpos-x_npos+x_mvel-x_nvel),mi)+pow((y_mpos-y_npos+y_mvel-y_nvel),mi));
